...
"""
    d = outchannel
    r = bit-rate [最终输出的channel数-用来表示使用多少个bit来存储图像]
"""
...
def train(beta,train_epoch):
    ...
    datasets.EMNIST('data', train=True, download=True, transform=transform, split='letters'),
    ...
    # TRAIN_PATH = "/content/gdrive/MyDrive/Colab Notebooks/simpson_results/train"
    ...
    for epoch in range(train_epoch): # 100
        D_losses = []
        G_losses = []
        G1_mse = []
        G1_2_mse = []
        ...
        for x_, y_ in train_loader:
            # train generator1 G1 (Gd--only use mse)
            G1.zero_grad()
            mini_batch = x_.size()[0]
            
            x_ = Variable(x_.cuda())
            x_mse_, bitstream = G1(x_) # bitstrean（encoder的输出）；x_mse_（decoder G_d的输出）
    ...
    # breakpoint()
    torch.save(G1.state_dict(),root+"EMNIST-%.2f"%(beta)+'-generator1_param.pkl')
    torch.save(G2.state_dict(), root+"EMNIST-%.2f"%(beta) + '-generator2_param.pkl')
    torch.save(D.state_dict(), root + "EMNIST-%.2f"%(beta)+ '-discriminator_param.pkl')
    with open(root + model + 'train_hist.pkl', 'wb') as f:
        pickle.dump(train_hist, f)

    show_train_hist(train_hist, save=True, path=root + "EMNIST-%.2f"%(beta) + '-train_hist.png')
    show_train_mse(train_hist, save=True, path=root + "EMNIST%.2f"%(beta) + '-train_mse.png')
    #ave(root + model + 'generation_animation.gif', images, fps=5)
    np.savetxt(root + "EMNIST-%.2f"%(beta)+'-G1_mse.txt', train_hist['G1_mse'], delimiter=" ")
    np.savetxt(root + "EMNIST-%.2f"%(beta)+'-G2_mse.txt', train_hist['G2_mse'], delimiter=" ")
